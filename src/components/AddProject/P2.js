...
import { PiMicrosoftExcelLogoFill } from 'react-icons/pi';  // Import the icon
...
const P2 = () => {
  ...
  const renderUploadedFiles = (cardTitle) => {
    ...
    return (
      <div className="relative flex items-center justify-start w-auto py-6 mt-8 ">
  ...
  const renderCard = (title, icon) => (
    <div className=''>
      <div className="flex items-center mb-2">
        {icon && <i className={`mr-2 ${icon} text-2xl text-blue-500`}></i>}
        <h2 className="text-lg font-bold text-gray-700">{title}</h2>
      </div>
      <div className="flex flex-col items-center p-6 border border-gray-300 border-dashed rounded-lg shadow-md">
        <img src={UploadSvg} alt="Upload" className="w-12 h-12 text-gray-400" />
        <div className='flex flex-row gap-2'>
          <label className="mb-2 text-blue-500 cursor-pointer " htmlFor={`${title}-file-upload`}>
            Click to upload
            <input type="file" id={`${title}-file-upload`} className="hidden" onChange={(e) => handleFileUpload(e, title)} />
          </label>
  ...
  return (
    <div className="container z-10 px-3 mx-auto mt-6 ">
      <ToastContainer />
      {/* Top Section */}
      <div className="flex flex-col items-center justify-between p-6 mt-16 border border-black shadow-md md:flex-row rounded-2xl">
      ...
      <div className="grid grid-cols-1 gap-6 mt-10 md:grid-cols-3">
        ...
        <div className="flex items-center p-2 border-gray-400 rounded-lg w-52 md:col-span-3 ml-96 border-3 ">
  <PiMicrosoftExcelLogoFill className="ml-10 mr-4 text-5xl text-green-500" />
  <h2 className="mt-3 mb-4 text-lg text-black">V.2.3.0.exl</h2>
</div>
